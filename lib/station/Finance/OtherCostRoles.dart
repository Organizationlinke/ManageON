...
// //             'CartonCost': double.tryParse(row[4]?.value.toString() ?? '0'),
// //             'PalletCost': double.tryParse(row[5]?.value.toString() ?? '0'),
// //             'CornerCost': double.tryParse(row[6]?.value.toString() ?? '0'),
// //             'WaxCost': double.tryParse(row[7]?.value.toString() ?? '0'),
// //             'SheetCost': double.tryParse(row[8]?.value.toString() ?? '0'),
// //             'PalletCoverCost': double.tryParse(row[9]?.value.toString() ?? '0'),
// //             'ChamberCost': double.tryParse(row[10]?.value.toString() ?? '0'),
// //             'NetCost': double.tryParse(row[11]?.value.toString() ?? '0'),
// //             'GlueCost': double.tryParse(row[12]?.value.toString() ?? '0'),
// //             'AdhesiveTabCost': double.tryParse(row[13]?.value.toString() ?? '0'),
// //             'WagesCost': double.tryParse(row[14]?.value.toString() ?? '0'),
// //             'ShippingCost': double.tryParse(row[15]?.value.toString() ?? '0'),
// //             'UtilitiesCost': double.tryParse(row[16]?.value.toString() ?? '0'),
// //             'FixedCost': double.tryParse(row[17]?.value.toString() ?? '0'),
// //             'WrapingCost': double.tryParse(row[18]?.value.toString() ?? '0'),
// //             'SticerCost': double.tryParse(row[19]?.value.toString() ?? '0'),
// //           };

// //           if (_checkOverlap(rowData)) {
// //             _showSnackBar('تنبيه: تداخل في التاريخ لـ ${rowData['CTNType']}، سيتم التخطي', Colors.orange);
// //             continue;
// //           }
...
// //             onConflict: 'DateFrom, DateTo, CTNWeight, CTNType',
// //           );
...
// //     String header = "DateFrom\tDateTo\tCTNType\tCTNWeight\tCartonCost\tPalletCost\tCornerCost\tWaxCost\tSheetCost\tPalletCoverCost\tChamberCost\tNetCost\tGlueCost\tAdhesiveTabCost\tWagesCost\tShippingCost\tUtilitiesCost\tFixedCost\tWrapingCost\tSticerCost\n";
// //     String body = _filteredData.map((row) {
// //       return "${row['DateFrom']}\t${row['DateTo']}\t${row['CTNType']}\t${row['CTNWeight']}\t${row['CartonCost']}\t${row['PalletCost']}\t${row['CornerCost']}\t${row['WaxCost']}\t${row['SheetCost']}\t${row['PalletCoverCost']}\t${row['ChamberCost']}\t${row['NetCost']}\t${row['GlueCost']}\t${row['AdhesiveTabCost']}\t${row['WagesCost']}\t${row['ShippingCost']}\t${row['UtilitiesCost']}\t${row['FixedCost']}\t${row['WrapingCost']}\t${row['SticerCost']}";
// //     }).join("\n");
...
// //               DataColumn(label: Text('ت.التغليف')),
// //               DataColumn(label: Text('ت.الاستيكرات')),
// //             ],
...
// //               DataCell(Text(item['WrapingCost']?.toString() ?? '0')),
// //               DataCell(Text(item['SticerCost']?.toString() ?? '0')),
// //             ])).toList(),
// //           ),
// //         ),
// //       ),
...
//   // دالة مساعدة لتقريب الأرقام لخانة عشرية واحدة أو اثنتين مع معالجة القيم الفارغة
...
//           if (_checkOverlap(rowData)) continue;
//           rowsToUpload.add(rowData);
//         }

//         if (rowsToUpload.isNotEmpty) {
//           await supabase.from('OtherCostRoles').upsert(rowsToUpload, onConflict: 'DateFrom, DateTo, CTNWeight, CTNType');
//           _showSnackBar('تمت معالجة ${rowsToUpload.length} سجل بنجاح', Colors.green);
//           _fetchData();
//         }
//       } catch (e) {
...
//       DateTime newTo = DateTime.parse(newRow['DateTo']);
//       for (var existing in _data) {
//         if (existing['CTNWeight'] == newRow['CTNWeight'] && existing['CTNType'] == newRow['CTNType']) {
//           DateTime exFrom = DateTime.parse(existing['DateFrom']);
//           DateTime exTo = DateTime.parse(existing['DateTo']);
//           if (newFrom == exFrom && newTo == exTo) continue;
//           if (newFrom.isBefore(exTo.add(const Duration(days: 1))) && exFrom.isBefore(newTo.add(const Duration(days: 1)))) return true;
//         }
//       }
...
//     String header = "DateFrom\tDateTo\tCTNType\tCTNWeight\tCartonCost\tPalletCost\tCornerCost\tWaxCost\tSheetCost\tPalletCoverCost\tChamberCost\tNetCost\tGlueCost\tAdhesiveTabCost\tWagesCost\tShippingCost\tUtilitiesCost\tFixedCost\tWrapingCost\tSticerCost\n";
//     String body = _filteredData.map((row) {
//       return "${row['DateFrom']}\t${row['DateTo']}\t${row['CTNType']}\t${row['CTNWeight']}\t${_formatNumber(row['CartonCost'])}\t${_formatNumber(row['PalletCost'])}\t${_formatNumber(row['CornerCost'])}\t${_formatNumber(row['WaxCost'])}\t${_formatNumber(row['SheetCost'])}\t${_formatNumber(row['PalletCoverCost'])}\t${_formatNumber(row['ChamberCost'])}\t${_formatNumber(row['NetCost'])}\t${_formatNumber(row['GlueCost'])}\t${_formatNumber(row['AdhesiveTabCost'])}\t${_formatNumber(row['WagesCost'])}\t${_formatNumber(row['ShippingCost'])}\t${_formatNumber(row['UtilitiesCost'])}\t${_formatNumber(row['FixedCost'])}\t${_formatNumber(row['WrapingCost'])}\t${_formatNumber(row['SticerCost'])}";
//     }).join("\n");
...
//       var excel = Excel.createExcel();
//       Sheet sheetObject = excel['Sheet1'];
//       List<String> headers = ['DateFrom', 'DateTo', 'CTNType', 'CTNWeight', 'CartonCost', 'PalletCost', 'CornerCost', 'WaxCost', 'SheetCost', 'PalletCoverCost', 'ChamberCost', 'NetCost', 'GlueCost', 'AdhesiveTabCost', 'WagesCost', 'ShippingCost', 'UtilitiesCost', 'FixedCost','WrapingCost','SticerCost'];
//       sheetObject.appendRow(headers.map((e) => TextCellValue(e)).toList());
//       var fileBytes = excel.save();
...
//           bytes: Uint8List.fromList(fileBytes!),
//         );
//         if (outputFile != null) _showSnackBar('تم حفظ القالب بنجاح', Colors.green);
//       }
//     } catch (e) {
//       _showSnackBar('خطأ أثناء إنشاء القالب: $e', Colors.red);
//     } finally {
...
//     return Theme(
//       data: Theme.of(context).copyWith(dividerColor: Colors.grey[300]),
//       child: Scrollbar( // إضافة شريط تمرير مرئي
...
//                 DataCell(Text(item['CTNWeight'] ?? '')),
//                 DataCell(Text(item['CTNType'] ?? '')),
//                 // استخدام دالة التقريب هنا لجميع خانات التكلفة
//                 DataCell(Text(_formatNumber(item['CartonCost']))),
//                 DataCell(Text(_formatNumber(item['PalletCost']))),
//                 DataCell(Text(_formatNumber(item['CornerCost']))),
//                 DataCell(Text(_formatNumber(item['WaxCost']))),
//                 DataCell(Text(_formatNumber(item['SheetCost']))),
//                 DataCell(Text(_formatNumber(item['PalletCoverCost']))),
//                 DataCell(Text(_formatNumber(item['ChamberCost']))),
//                 DataCell(Text(_formatNumber(item['NetCost']))),
//                 DataCell(Text(_formatNumber(item['GlueCost']))),
//                 DataCell(Text(_formatNumber(item['AdhesiveTabCost']))),
//                 DataCell(Text(_formatNumber(item['WagesCost']))),
//                 DataCell(Text(_formatNumber(item['ShippingCost']))),
//                 DataCell(Text(_formatNumber(item['UtilitiesCost']))),
//                 DataCell(Text(_formatNumber(item['FixedCost']))),
//                 DataCell(Text(_formatNumber(item['WrapingCost']))),
//                 DataCell(Text(_formatNumber(item['SticerCost']))),
//               ])).toList(),
//             ),
//           ),
//         ),
...
class _OtherCostRolesScreenState extends State<OtherCostRolesScreen> {
  ...
  // 1. تعريف المتحكمات لحل مشكلة الـ Scrollbar
  ...
  Widget _buildDataTable() {
    return Theme(
      data: Theme.of(context).copyWith(dividerColor: Colors.grey[300]),
      child: Scrollbar(
        controller: _verticalController, // ربط المتحكم الرأسي هنا
        thumbVisibility: true,
        child: SingleChildScrollView(
          controller: _verticalController, // ربط المتحكم الرأسي هنا أيضاً
          scrollDirection: Axis.vertical,
          child: Scrollbar(
            controller: _horizontalController, // ربط المتحكم الأفقي هنا
            thumbVisibility: true,
            notificationPredicate: (notif) => notif.depth == 1, // لمنع تداخل الأحداث
            child: SingleChildScrollView(
              controller: _horizontalController, // ربط المتحكم الأفقي هنا أيضاً
              scrollDirection: Axis.horizontal,
              child: DataTable(
