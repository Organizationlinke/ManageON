...
class ColdStorReportScreen extends StatefulWidget {
  ...
  @override
  State<ColdStorReportScreen> createState() => _ColdStorReportScreenState();
}

class _ColdStorReportScreenState extends State<ColdStorReportScreen> {
  final supabase = Supabase.instance.client;
  ...
  List rawReportData = []; // البيانات الخام
  ...
  // مخزن الفلاتر النشطة (فلترة داخل الجدول)
  ...
  Future<void> _loadSavedColumns() async {
    ...
    setState(() {
      selectedTextCols = prefs.getStringList('coldstor_selectedTextCols') ?? [];
      selectedNumCols = prefs.getStringList('coldstor_selectedNumCols') ?? [];
    });
  }
  ...
  Future<void> _saveColumns() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList('coldstor_selectedTextCols', selectedTextCols);
    await prefs.setStringList('coldstor_selectedNumCols', selectedNumCols);
  }
  ...
  void _processAndMergeData() {
    ...
    for (var row in rawReportData) {
      ...
      // مفتاح فريد يعتمد على الصنف والدرجة والأعمدة النصية الظاهرة
  ...
  // تطبيق الفلاتر المختارة من رؤوس الأعمدة
  ...
  // عرض نافذة الفلترة الخاصة بكل عمود
  ...
  Widget _buildFilterHeader(String title, String colKey) {
    ...
    return InkWell(
      ...
      child: Row(
        ...
        children: [
          Text(title),
          Icon(Icons.filter_alt, size: 14, color: isFiltered ? Colors.blue : Colors.grey[400]),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        backgroundColor: Colors.grey[100],
        appBar: AppBar(
          title: const Text('ارصدة الثلاجة'),
          centerTitle: true,
          actions: [
            ...
            Row(
              ...
              children: [
                ...
                TextButton.icon(
                  ...
                  label: const Text('تصفية', style: TextStyle(fontSize: 12)),
                ),
              ],
            ),
          ],
  ...
  Widget _summaryCards() {
    return Padding(
      ...
      child: Row(
        children: [
          _cardInfo('إجمالي الكمية المعروضة', formatNum(totalNetWeight), Colors.blue),
        ],
      ),
    );
  }
  ...
  Widget _tableSection() {
    if (filteredData.isEmpty && !loading) return const Center(child: Text('لا توجد بيانات تطابق البحث'));
          ...
          child: SingleChildScrollView(
            ...
            child: DataTable(
              ...
              columns: [
                ...
                const DataColumn(label: Text('الكمية الصافية')),
              ...
              rows: [
                ...
                DataRow(
                  ...
                  cells: [
                    const DataCell(Text('الإجمالي', style: TextStyle(fontWeight: FontWeight.bold))),
                    const DataCell(Text('')),
                    DataCell(Text(formatNum(totalNetWeight), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue))),
                    ...selectedTextCols.map((_) => const DataCell(Text(''))),
                    ...selectedNumCols.map((c) => DataCell(
