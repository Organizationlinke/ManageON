...
import 'package:flutter/services.dart'; // ضروري للحافظة
import 'package:intl/intl.dart' as intl;
import 'package:supabase_flutter/supabase_flutter.dart';
import 'package:shared_preferences/shared_preferences.dart';

class SalesInvoicewithCosts extends StatefulWidget {
  const SalesInvoicewithCosts({super.key});

  @override
  State<SalesInvoicewithCosts> createState() => _SalesInvoiceReportScreenState();
}

class _SalesInvoiceReportScreenState extends State<SalesInvoicewithCosts> {
  ...
  final Map<String, String> numericColumnsMap2 = {
    'GrossWeight': 'الوزن القائم',
    'CTNCountInPallets': 'عدد الكرتون',
    'Pallet_Count': 'عدد البالتات',
    'AMT_USD_FOB': 'قيمة المبيعات الاصليه',
    'RawCost': 'ت الخام',
    'CartonCost': 'ت الكرتون',
    'TotalOtherPackingCost': 'ت مستلزمات اخري',
    'WaxCost': 'ت الشموع والمطهرات',
    'UtilitiesCost': 'ت المياه والكهرباء',
    'WagesCost': 'ت الاجور',
    'ShippingCost': 'ت الشحن',
    'FixedCost': 'ت الثابتة',
    'TotalCost': 'اجمالي التكلفة',
    'HamaProfit': 'ربحية هامه',
    'AllProfit': 'اجمالي الربحية',
  };
  ...
  // --- دالة نسخ البيانات إلى الإكسل (عبر الحافظة) ---
  Future<void> _copyToExcel() async {
    ...
    List<String> headers = [
      ...
      ...selectedNumCols.map((c) => numericColumnsMap2[c] ?? c),
    ];
    ...
    for (var row in filteredData) {
      ...
      // تقريب عدد الحاويات لرقمين عشريين
      num containerCount = row['container_count'] ?? 0;

      List<String> rowData = [
        row['crop_name']?.toString() ?? '',
        row['class']?.toString() ?? '',
        rowW.toString(),
        (rowW == 0 ? 0 : rowV / rowW).toStringAsFixed(2),
        rowV.toString(),
        containerCount.toStringAsFixed(2), // تعديل هنا
    ...
    List<String> totalRow = [
      'الإجمالي',
      '',
      currentNetWeight.toString(),
      avgPrice.toStringAsFixed(2),
      currentValue.toString(),
      currentContainers.toStringAsFixed(2), // تعديل هنا
      ...selectedTextCols.map((_) => ''),
      ...selectedNumCols.map((c) {
        num sum = filteredData.fold<num>(0.0, (s, e) {
    ...
    csvContent.writeln(totalRow.join('\t'));
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('تم نسخ البيانات! يمكنك الآن لصقها (Paste) في ملف Excel مباشرة.'),
  ...
  Future<void> _loadSavedSettings() async {
    ...
    setState(() {
      selectedTextCols = prefs.getStringList('sales_selectedTextCols2') ?? [];
      selectedNumCols = prefs.getStringList('sales_selectedNumCols2') ?? [];
    });
  }

  Future<void> _saveSettings() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList('sales_selectedTextCols2', selectedTextCols);
    await prefs.setStringList('sales_selectedNumCols2', selectedNumCols);
  }

  // تم تعديل دالة التنسيق لتقبل التحكم في عدد الأرقام العشرية
  String formatNum(dynamic v, {bool isMoney = true, int decimalPlaces = 2}) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              onPressed: _copyToExcel,
              icon: const Icon(Icons.copy_all_rounded),
              tooltip: 'نسخ لملف إكسل',
            ),
          ],
        ),
        body: Column(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // تم استخدام formatNum مع تمرير decimalPlaces: 2 لعمود الحاويات
          _cardInfo('الحاويات', formatNum(currentContainers, isMoney: false, decimalPlaces: 2), Colors.orange),
        ],
      ),
    );
  }
          ...
          child: SingleChildScrollView(
            ...
            child: DataTable(
              ...
              columns: [
                ...
                ...selectedNumCols.map((c) => DataColumn(label: Text(numericColumnsMap2[c] ?? c))),
              ],
              rows: [
                ...filteredData.map((row) {
                  ...
                  return DataRow(cells: [
                    ...
                    // تعديل هنا: عرض الحاويات برقمين عشريين
                    DataCell(Text(formatNum(row['container_count'], isMoney: false, decimalPlaces: 2))),
                ...
                DataRow(
                  ...
                  cells: [
                    ...
                    // تعديل هنا: إجمالي الحاويات برقمين عشريين
                    DataCell(Text(formatNum(currentContainers, isMoney: false, decimalPlaces: 2), style: const TextStyle(fontWeight: FontWeight.bold))),
  ...
  // بقية الدوال (الفلاتر، التاريخ، الخ) تظل كما هي...
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(builder: (context, setL) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          ...numericColumnsMap2.entries.map((e) => CheckboxListTile(title: Text(e.value), value: selectedNumCols.contains(e.key), onChanged: (v) => setL(() => v! ? selectedNumCols.add(e.key) : selectedNumCols.remove(e.key)))),
        ])),
